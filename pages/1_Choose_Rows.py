...
for protocol in active_protocols:
    ...
    st.dataframe(df, use_container_width=True)  # Always show raw table first
    ...
    if default_rename >= len(df):
        st.warning(f"⚠️ Saved rename_row {default_rename} out of bounds. Resetting to 0.")
        default_rename = 0
...
if st.button("📊 Save Selections"):
    ...
    if rowcol_data:
        ...
        try:
            ...
            # 🔍 Verify Supabase write worked
            verify_df = storage.get_row_col_map()
            missing = [p for p in active_protocols if p not in verify_df["protocol"].unique()]
            if missing:
                st.warning(f"⚠️ These protocols did NOT save correctly: {', '.join(missing)}")
        except Exception as e:
